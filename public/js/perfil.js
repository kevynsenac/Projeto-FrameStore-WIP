...
async function carregarDadosUsuario() {
  ...
  document.documentElement.style.setProperty("--cor-tema-rgb", hexToRgb(corTema));

  const bannerElement = document.querySelector(".profile-header");
  const pageBackground = document.getElementById("page-background");

  if (usuarioLogado.fundo_perfil) {
    if (bannerElement) bannerElement.style.backgroundImage = `url('${usuarioLogado.fundo_perfil}')`;
    if (pageBackground) pageBackground.style.backgroundImage = `url('${usuarioLogado.fundo_perfil}')`;
  } else {
    if (bannerElement) bannerElement.style.background = `linear-gradient(135deg, #1a1a2e 0%, ${corTema}40 100%)`;
  ...
  document.getElementById("email-usuario").innerHTML = `<i class="fas fa-envelope"></i> ${usuarioLogado.email}`;

  const bioEl = document.getElementById("bio-usuario");
  if (bioEl) {
    bioEl.innerText = usuarioLogado.bio || "Perfil sem bio. Vá em 'Minha Conta' para alterar!";
  }
...
async function carregarBiblioteca() {
  try {
    ...
    document.getElementById("biblioteca-container").innerHTML = "<p>Erro ao carregar jogos.</p>";
  }
}
    ...
    cupons.forEach((cupom) => {
      ...
      li.innerHTML = `
        <div>
          ...
          <p style="font-size: 0.9rem; color: #ccc;">Desconto</p>
        </div>
...
async function salvarEdicaoPerfil(senhaAtual) {
  ...
  try {
    ...
    document.getElementById("label-fundo").style.borderColor = "var(--cor-tema)";
...
function abrirModalJogo(game) {
  ...
  document.getElementById("modal-game-cover").src = game.cover || "img/site_logo.png";
  document.getElementById("modal-game-date").innerText = new Date(game.data_compra).toLocaleDateString("pt-BR");
  document.getElementById("modal-game-code").innerText = game.codigo_resgate || "Erro no código.";
...
function copiarChaveAtivacao(btnElement) {
  ...
  navigator.clipboard.writeText(codigo)
    .then(() => {
      mostrarNotificacao("Chave copiada para a área de transferência!", "sucesso");
